...
namespace BuildXL.Processes
{
    ...
    public unsafe class JobObject : SafeHandleZeroOrMinusOneIsInvalid
    {
        ...
        private uint ComputeRequiredBufferSizeForProcessIdListLength(uint length) => (uint)(Marshal.SizeOf<JOBOBJECT_BASIC_PROCESS_ID_LIST>() + (UIntPtr.Size * (length - 1)));
        ...
        private bool TryGetProcessIds(LoggingContext loggingContext, uint length, out uint[]? processIds, out uint requiredLength)
        {
            ...
            uint bufferSizeInBytes = ComputeRequiredBufferSizeForProcessIdListLength(length);
            uint bufferSizeInEntries = (bufferSizeInBytes + sizeof(ulong) - 1) / sizeof(ulong);
