...
void WINAPI DetourInit() {
    DetourInitTracing();
    DETOUR_TRACE_INFO(L"Detours initialized: Process ID %d", GetCurrentProcessId());
}
...
LONG WINAPI DetourTransactionBegin()
{
    ...
    if (s_nPendingThreadId != 0) {
        DETOUR_TRACE_ERROR(L"Only one transaction is allowed at a time.\n");
        return ERROR_INVALID_OPERATION;
    }
    // Make sure only one thread can start a transaction.
    if (InterlockedCompareExchange(&s_nPendingThreadId, (LONG)GetCurrentThreadId(), 0) != 0) {
        DETOUR_TRACE_ERROR(L"Make sure only one thread can start a transaction.\n");
        return ERROR_INVALID_OPERATION;
    }
...
LONG WINAPI DetourTransactionAbort()
{
    if (s_nPendingThreadId != (LONG)GetCurrentThreadId()) {
        DETOUR_TRACE_ERROR(L"transaction conflict with thread id=%d\n", s_nPendingThreadId);
        return ERROR_INVALID_OPERATION;
    }
...
LONG WINAPI DetourTransactionCommitEx(PVOID **pppFailedPointer)
{
    ...
    if (s_nPendingError != NO_ERROR) {
        DETOUR_TRACE_ERROR(L"pending transaction=%d\n", s_nPendingError);
        DETOUR_BREAK();
    ...
    for (DetourOperation *o = s_pPendingOperations; o != NULL; o = o->pNext) {
        if (o->fIsRemove) {
            ...
            *o->ppbPointer = DETOURS_PBYTE_TO_PFUNC(o->pbTarget);
            ...
            DETOUR_TRACEW((L"detours: pbTarget=%p: "
                          L"%02x %02x %02x %02x "
                          L"%02x %02x %02x %02x "
                          L"%02x %02x %02x %02x [before]\n",
                          o->pbTarget,
            ...
            *o->ppbPointer = (PBYTE)&o->pTrampoline->pldTrampoline;
            ...
            detour_gen_brk(pbCode, o->pTrampoline->pbRemain);
            ...
            *o->ppbPointer = DETOURS_PBYTE_TO_PFUNC(o->pTrampoline->rbCode);
            ...
            DETOUR_TRACEW((L"detours: pbTarget=%p: "
                          L"%02x %02x %02x %02x "
                          L"%02x %02x %02x %02x "
                          L"%02x %02x %02x %02x [after]\n",
                          o->pbTarget,
                          o->pbTarget[0], o->pbTarget[1], o->pbTarget[2], o->pbTarget[3],
                          o->pbTarget[4], o->pbTarget[5], o->pbTarget[6], o->pbTarget[7],
                          o->pbTarget[8], o->pbTarget[9], o->pbTarget[10], o->pbTarget[11]));

            DETOUR_TRACEW((L"detours: pbTramp =%p: "
                          L"%02x %02x %02x %02x "
                          L"%02x %02x %02x %02x "
                          L"%02x %02x %02x %02x\n",
                          o->pTrampoline,
    ...
    for (DetourThread *t = s_pPendingThreads; t != NULL; t = t->pNext) {
        CONTEXT cxt;
...
LONG WINAPI DetourUpdateThread(HANDLE hThread)
{
    ...
    if (s_nPendingError != NO_ERROR) {
        DETOUR_TRACE_ERROR(L"pending transaction error=%d\n", s_nPendingError);
        return s_nPendingError;
    }
    ...
    if (SuspendThread(hThread) == (DWORD)-1) {
        DETOUR_TRACE_ERROR(L"SuspendThread(%p) failed: %d\n",
                           ...
                           PVOID *ppRealDetour)
{
    ...
    if (ppPointer == NULL) {
        DETOUR_TRACE_ERROR(L"ppPointer is null\n");
    ...
    if (*ppPointer == NULL) {
        ...
        DETOUR_TRACE_ERROR(L"*ppPointer is null (ppPointer=%p)\n", ppPointer);
        DETOUR_BREAK();
        return error;
    }
    ...
    if (pDetour == (PVOID)pbTarget) {
        DETOUR_TRACE_ERROR(L"pDetour == (PVOID)pbTarget\n");
    ...
    if (pTrampoline == NULL) {
        DETOUR_TRACE_ERROR(L"pTrampoline == NULL\n");
        error = ERROR_NOT_ENOUGH_MEMORY;
        DETOUR_BREAK();
        goto fail;
    }
    ...
    if (cbTarget < cbJump || nAlign > ARRAYSIZE(pTrampoline->rAlign)) {
        ...
        DETOUR_TRACE_ERROR(L"Too few instructions: cbTarget < cbJump -> ERROR_INVALID_BLOCK\n");
    ...
    if (cbTarget > sizeof(pTrampoline->rbCode) - cbJump) {
        DETOUR_TRACE_ERROR(L"Too many instructions.\n");
        error = ERROR_INVALID_HANDLE;
        DETOUR_BREAK();
        goto fail;
    }
    ...
    detour_gen_brk(pbTrampoline, pbPool);
#endif // DETOURS_X64
    ...
    if (!VirtualProtect(pbTarget, cbTarget, PAGE_EXECUTE_READWRITE, &dwOld)) {
        DETOUR_TRACE_ERROR(L"VirtualProtect(%p) failed: %d\n",
            pbTarget, GetLastError());
        error = GetLastError();
        DETOUR_BREAK();
        goto fail;
    }
                         ...
                         PVOID pDetour)
{
    ...
    if (ppPointer == NULL) {
        DETOUR_TRACE_ERROR(L"ppPointer == NULL\n");
        return ERROR_INVALID_HANDLE;
    }
    if (*ppPointer == NULL) {
        DETOUR_TRACE_ERROR(L"*ppPointer == NULL\n");
    ...
    if (cbTarget == 0 || cbTarget > sizeof(pTrampoline->rbCode)) {
        DETOUR_TRACE_ERROR(L"Detach: Trampoline looks too small -> ERROR_INVALID_BLOCK\n");
        error = ERROR_INVALID_BLOCK;
        if (s_fIgnoreTooSmall) {
            goto stop;
        }
    ...
    if (pTrampoline->pbDetour != pDetour) {
        DETOUR_TRACE_ERROR(L"Detach: Trampoline points to the wrong place -> ERROR_INVALID_BLOCK\n");
        error = ERROR_INVALID_BLOCK;
        if (s_fIgnoreTooSmall) {
            goto stop;
        }
